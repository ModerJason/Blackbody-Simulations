...
def setup_plane_wave(plane_wave_face, i_theta_lower, i_theta_upper, i_phi_lower, i_phi_upper,
    ...
    # For the polarization, θ is (cosθcosϕ, cosθsinϕ, -sinθ). ϕ = (-sinϕ, cosϕ, 0)
    hfss.plane_wave(
...
def run_analysis(num_cores, max_delta_E, max_passes, plane_wave_face, Ei, output_file_location, i_theta_lower, i_theta_upper, i_phi_lower, i_phi_upper,
                 frequency, i_theta_step, i_phi_step, rad_params, adaptive=True, max_difference = 0.02):
    ...
    if adaptive:
        ...
        # We do not reuse the same setups because for we will reuse the meshes for new frequencies
        parametric_setup_name = get_parametric_setup_name(frequency)
...
def main():
    ...
    i_theta_lower, i_theta_upper, i_phi_lower, i_phi_upper = 90, 180, 0, 360
    ...
    # Updates:
    # (1) Adaptive sweep and merging
    # (2) Might be better to run a relatively refined sweep to begin with, or else there are simply too many regions to refine

    # Future plans:
    # (1) Make easier to read from CSV directly
    # (2) On a similar note, make it so that it doesn't give errors if there are duplicates. More assertions
    # everything except for initialize_variables, others check and try/except loops
